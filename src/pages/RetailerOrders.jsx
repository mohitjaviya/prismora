...
export default function RetailerOrders() {
  const { orders, addOrder, confirmOrderReceipt, productCatalog, schemes, territories, retailers, dealers, distributors } = useData();
  const { user } = useAuth();
  ...
  const handleSubmitOrder = () => {
    ...
    const territory = territoryFor(territories, retailer)
      ...
      || territoryForPlace(territories, retailer).territory;
    const parentDealer = dealers.find(d => d.id === retailer?.parentDealerId);

    // Retailer, then their dealer, then that dealer's distributor.
    ...
    const routing = assigneeForPortalOrder([retailer, parentDealer, distributors.find(d => d.id === parentDealer?.parentDistributorId)], territories);
    addOrder({
      ...
      deliveryPincode: retailer.pincode || '',
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
              <div className="flex gap-3 items-end">
                <div className="flex-1">
                  <label htmlFor="retailerorders-product" className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">Product</label>
                  <select id="retailerorders-product" value={selectedProduct} onChange={e => setSelectedProduct(e.target.value)} className="w-full glass-input rounded-xl px-4 py-2.5 text-sm text-white">
                ...
                <div className="w-28">
                  <label htmlFor="retailerorders-qty" className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">Qty</label>
                  <input id="retailerorders-qty" type="number" min="1" value={qty} onChange={e => setQty(e.target.value)} className="w-full glass-input rounded-xl px-4 py-2.5 text-sm text-white" />
                </div>
