...
import {
  ShoppingBag, Plus, Edit2, Trash2, Search, X, Download,
  Phone, Mail, MapPin,
  CreditCard, IndianRupee, Eye, ShieldCheck, ShieldX, Wallet, ArrowUpCircle, ArrowDownCircle, Network
} from 'lucide-react';
...
const BLANK_FORM = {
  ...
  phone: '', email: '', contactPerson: '', creditLimit: 50000, status: 'Active'
};
...
export default function Retailers() {
  ...
  const openEdit = (r) => { setEditingRetailer(r); setForm({ name: r.name, gstin: r.gstin || '', parentDealerId: r.parentDealerId || '', state: r.state || '', city: r.city || '', territory: r.territory || '', phone: r.phone || '', email: r.email || '', contactPerson: r.contactPerson || '', creditLimit: r.creditLimit || 50000, status: r.status || 'Active' }); setIsModalOpen(true); };
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <ShoppingBag size={24} className="text-brand-accent" /> Retailer Management
          </h1>
          <p className="text-slate-400 text-sm mt-1">Retailers linked to dealers, territory mapping & outstanding ledger.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => downloadCSV(filtered.map(r => ({ Name: r.name, GSTIN: r.gstin, ParentDealer: parentDealerName(r.parentDealerId), State: r.state, City: r.city, Territory: r.territory, Phone: r.phone, Email: r.email, Outstanding: r.outstandingAmount, CreditLimit: r.creditLimit, Status: r.status })), 'PRISMORA_Retailers')}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          { label: 'Total Retailers', value: kpis.total, color: 'text-blue-400' },
          ...
          <table className="w-full text-left text-sm border-collapse">
            ...
            <tbody className="divide-y divide-white/5 text-slate-300">
              {filtered.length > 0 ? filtered.map(r => {
                const isOverLimit = (r.outstandingAmount || 0) > (r.creditLimit || 50000);
                const utilizationPct = r.creditLimit ? Math.min(100, Math.round(((r.outstandingAmount || 0) / r.creditLimit) * 100)) : 0;
                return (
                  <tr key={r.id} className="hover:bg-brand-primary-lighter/20 transition-colors">
                    <td className="p-4">
                      <div className="font-semibold text-white">{r.name}</div>
                      <div className="text-xs text-slate-500 font-mono">{r.gstin || '—'}</div>
                    </td>
                    <td className="p-4">
                      <div className="text-sm flex items-center gap-1.5"><Network size={12} className="text-brand-accent flex-shrink-0" />{parentDealerName(r.parentDealerId)}</div>
                    </td>
                    <td className="p-4">
                      <div className="text-sm">{r.territory || '—'}</div>
                    ...
                    <td className="p-4 text-right">
                      <span className={`font-bold ${isOverLimit ? 'text-rose-400' : 'text-white'}`}>{formatCurrency(r.outstandingAmount || 0)}</span>
                    ...
                    <td className="p-4 text-right text-slate-400">{formatCurrency(r.creditLimit)}</td>
                    <td className="p-4 text-center">
                      <span className={`px-2 py-1 rounded-full text-[10px] font-bold border ${r.status === 'Active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : r.status === 'Pending' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' : 'bg-slate-500/10 text-slate-400 border-slate-500/20'}`}>
                        {r.status}
                      </span>
                    </td>
                    <td className="p-4 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        {r.status === 'Pending' && canManage ? (
                          <>
                            <button onClick={() => approveRetailer(r)} className="p-1.5 text-slate-400 hover:text-emerald-400 hover:bg-emerald-400/10 rounded-lg transition-colors" title="Approve"><ShieldCheck size={14} /></button>
                            <button onClick={() => rejectRetailer(r)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors" title="Reject"><ShieldX size={14} /></button>
                          </>
                        ) : null}
                        <button onClick={() => setViewingRetailer(r)} className="p-1.5 text-slate-400 hover:text-blue-400 hover:bg-blue-400/10 rounded-lg transition-colors" title="View Details"><Eye size={14} /></button>
                        {canManage && <>
                          <button onClick={() => openEdit(r)} className="p-1.5 text-slate-400 hover:text-brand-accent hover:bg-brand-accent/10 rounded-lg transition-colors"><Edit2 size={14} /></button>
                          <button onClick={() => { if (confirm('Delete this retailer?')) deleteRetailer(r.id); }} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors"><Trash2 size={14} /></button>
                        </>}
                      </div>
                    </td>
                  </tr>
                ...
                <tr><td colSpan="8" className="p-12 text-center text-slate-500">
                  ...
                  {canManage && <button onClick={openAdd} className="mt-4 text-brand-accent hover:underline text-sm">+ Add your first retailer</button>}
                </td></tr>
              )}
            </tbody>
          </table>
