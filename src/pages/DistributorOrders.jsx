...
export default function DistributorOrders() {
  const { orders, addOrder, confirmOrderReceipt, productCatalog, schemes, territories, distributors } = useData();
  const { user } = useAuth();
  ...
  const myOrders = useMemo(() =>
    orders.filter(o => o.distributorId === distributor?.id)
      .sort((a, b) => new Date(b.createdAt || b.date) - new Date(a.createdAt || a.date)),
    [orders, distributor]
  );

  const cartTotal = cart.reduce((s, i) => s + i.quantity * i.distributorPrice, 0);
  const cartQty = cart.reduce((s, i) => s + i.quantity, 0);

  const qualifyingScheme = useMemo(() => {
    const cartOrder = { value: cartTotal, items: cart.map(i => ({ name: i.name, total: i.quantity * i.distributorPrice })) };
  ...
  const addToCart = () => {
    ...
    setCart(prev => {
      ...
      return [...prev, { id: product.id, name: product.name, distributorPrice: product.distributorPrice, gstPct: product.gstPct, uom: product.uom, quantity: Number(qty) }];
    });
    setSelectedProduct('');
    setQty('');
  };
  ...
  const handleSubmitOrder = () => {
    ...
    addOrder({
      ...
      quantity: cartQty,
      value: cartTotal,
      items: cart.map(i => ({ name: i.name, quantity: i.quantity, unitPrice: i.distributorPrice, gstPct: i.gstPct, total: i.quantity * i.distributorPrice })),
      ...
      <div className="space-y-6 animate-fade-in-up">
        ...
        <Card padding="p-0">
          <EmptyState
            ...
            hint="This login is not linked to a distributor record, so there are no orders to show. An administrator can link it from the Distributors screen."
          />
        </Card>
      </div>
    );
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-4">
              <div className="flex gap-3 items-end">
                <div className="flex-1">
                  <label htmlFor="distributororders-product" className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wide">Product</label>
                  <select id="distributororders-product" value={selectedProduct} onChange={e => setSelectedProduct(e.target.value)} className="w-full glass-input rounded-xl px-4 py-2.5 text-sm text-white">
                    <option value="" className="bg-brand-primary text-slate-500">-- Select Product --</option>
                    {productCatalog.map(p => <option key={p.id} value={p.id} className="bg-brand-primary">{p.name} — {formatCurrency(p.distributorPrice)}/{p.uom}</option>)}
                  </select>
                </div>
                <div className="w-28">
                    ...
                    <tbody className="divide-y divide-white/5 text-slate-300">
                      {cart.map(i => (
                        <tr key={i.id}>
                          ...
                          <td className="p-3 text-right">{formatCurrency(i.distributorPrice)}</td>
                          <td className="p-3 text-right font-medium text-white">{formatCurrency(i.quantity * i.distributorPrice)}</td>
