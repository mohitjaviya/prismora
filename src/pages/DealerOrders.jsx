...
export default function DealerOrders() {
  ...
  const myOrders = useMemo(() =>
    orders.filter(o => o.dealerId === dealer?.id)
      .sort((a, b) => new Date(b.createdAt || b.date) - new Date(a.createdAt || a.date)),
    [orders, dealer]
  );

  const cartTotal = cart.reduce((s, i) => s + i.quantity * i.dealerPrice, 0);
  const cartQty = cart.reduce((s, i) => s + i.quantity, 0);

  const qualifyingScheme = useMemo(() => {
    const cartOrder = { value: cartTotal, items: cart.map(i => ({ name: i.name, total: i.quantity * i.dealerPrice })) };
    return schemes.find(s =>
      ['Dealer', 'All'].includes(s.applicableTo) &&
  ...
  const addToCart = () => {
    ...
    setCart(prev => {
      ...
      return [...prev, { id: product.id, name: product.name, dealerPrice: product.dealerPrice, gstPct: product.gstPct, uom: product.uom, quantity: Number(qty) }];
    });
    setSelectedProduct('');
    setQty('');
  };
  ...
  const handleSubmitOrder = () => {
    ...
    const territory = territories.find(t => t.name === dealer.territory)
    ...
    addOrder({
      ...
      quantity: cartQty,
      value: cartTotal,
      items: cart.map(i => ({ name: i.name, quantity: i.quantity, unitPrice: i.dealerPrice, gstPct: i.gstPct, total: i.quantity * i.dealerPrice })),
      state: dealer.state,
      city: dealer.city,
      status: 'Pending',
      assignedTo: territory?.executiveId || '',
      territory: territory?.name || dealer.territory || '',
      deliveryAddress: dealer.address || '',
      deliveryPincode: dealer.pincode || '',
      dealerId: dealer.id,
      phone: dealer.phone,
      email: dealer.email,
    ...
    return (
      <div className="space-y-6 animate-fade-in-up">
        ...
        <Card padding="p-0">
          <EmptyState
            icon={UserX}
            title="Your dealer profile could not be found"
            hint="This login is not linked to a dealer record, so there are no orders to show. An administrator can link it from the Dealers screen."
          />
        </Card>
      </div>
    );
                    ...
                    <tbody className="divide-y divide-white/5 text-slate-300">
                      {cart.map(i => (
                        <tr key={i.id}>
                          ...
                          <td className="p-3 text-right">{formatCurrency(i.dealerPrice)}</td>
                          <td className="p-3 text-right font-medium text-white">{formatCurrency(i.quantity * i.dealerPrice)}</td>
