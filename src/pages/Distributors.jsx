...
import {
  ...
  Phone, Mail, MapPin, AlertCircle, CheckCircle,
  CreditCard, IndianRupee, Eye, ShieldCheck, ShieldX, Wallet, ArrowUpCircle, ArrowDownCircle
} from 'lucide-react';
...
export default function Distributors() {
  const { distributors, addDistributor, updateDistributor, deleteDistributor, invoices, distributorPayments, addDistributorPayment } = useData();
  ...
  const ledgerEntries = useMemo(() => viewingDist ? buildLedgerEntries(viewingDist, invoices, distributorPayments) : [], [viewingDist, invoices, distributorPayments]);
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Network size={24} className="text-brand-accent" /> Distributor Management
          </h1>
          <p className="text-slate-400 text-sm mt-1">Super stockists, distributors, territory mapping & outstanding ledger.</p>
        </div>
        <div className="flex gap-3">
          <button onClick={() => downloadCSV(filtered.map(d => ({ Name: d.name, GSTIN: d.gstin, State: d.state, City: d.city, Territory: d.territory, Phone: d.phone, Email: d.email, Outstanding: d.outstandingAmount, CreditLimit: d.creditLimit, Status: d.status })), 'PRISMORA_Distributors')}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          { label: 'Total Distributors', value: kpis.total, color: 'text-blue-400' },
            ...
            <tbody className="divide-y divide-white/5 text-slate-300">
              {filtered.length > 0 ? filtered.map(d => {
                    ...
                    <td className="p-4 text-center">
                      <div className="flex items-center justify-center gap-1.5">
                        {d.status === 'Pending' && canManage ? (
                          <>
                            <button onClick={() => approveDistributor(d)} className="p-1.5 text-slate-400 hover:text-emerald-400 hover:bg-emerald-400/10 rounded-lg transition-colors" title="Approve"><ShieldCheck size={14} /></button>
                            <button onClick={() => rejectDistributor(d)} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors" title="Reject"><ShieldX size={14} /></button>
                          </>
                        ) : null}
                        <button onClick={() => setViewingDist(d)} className="p-1.5 text-slate-400 hover:text-blue-400 hover:bg-blue-400/10 rounded-lg transition-colors" title="View Details"><Eye size={14} /></button>
                        {canManage && <>
                          ...
                          <button onClick={() => { if (confirm('Delete this distributor?')) deleteDistributor(d.id); }} className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-red-400/10 rounded-lg transition-colors"><Trash2 size={14} /></button>
                        </>}
                      </div>
                    </td>
                  </tr>
                ...
                <tr><td colSpan="7" className="p-12 text-center text-slate-500">
                  ...
                  {canManage && <button onClick={openAdd} className="mt-4 text-brand-accent hover:underline text-sm">+ Add your first distributor</button>}
                </td></tr>
              )}
            </tbody>
          </table>
          ...
          <div className="relative glass-panel bg-brand-primary w-full max-w-2xl max-h-[90vh] rounded-2xl shadow-2xl border border-brand-accent/30 animate-fade-in-up z-10 flex flex-col overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="sm:col-span-2"><label className={labelCls}>Company / Business Name *</label><input required type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="e.g. Gujarat Super Stockist" className={inputCls} /></div>
