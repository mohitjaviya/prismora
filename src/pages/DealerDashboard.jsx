...
export default function DealerDashboard() {
  const { orders, schemes, distributorIncentives, dealers } = useData();
  const { user } = useAuth();

  const dealer = useMemo(() => dealers?.find(d => d.id === user?.dealerId), [dealers, user]);

  const myOrders = useMemo(() => orders.filter(o => o.dealerId === dealer?.id), [orders, dealer]);
  ...
  const incentivesPending = useMemo(() =>
    distributorIncentives
      .filter(i => i.dealerId === dealer?.id && i.status === 'Earned' && i.incentiveType !== 'Free Goods')
      .reduce((s, i) => s + Number(i.incentiveValue || 0), 0),
    [distributorIncentives, dealer]
  );

  const utilizationPct = dealer?.creditLimit ? Math.min(100, Math.round(((dealer.outstandingAmount || 0) / dealer.creditLimit) * 100)) : 0;
      ...
      <div className="space-y-6 animate-fade-in-up">
        ...
        <Card padding="p-0">
          <EmptyState
            ...
            hint="This login is not linked to a dealer record, so there is no account to show. An administrator can link it from the Dealers screen — it takes a moment and nothing is lost in the meantime."
          />
        </Card>
      </div>
    );
  ...
  const kpiCards = [
    { label: 'Outstanding', tone: 'danger', value: formatCurrency(dealer.outstandingAmount || 0), icon: Wallet, sub: `${utilizationPct}% of ${formatCurrency(dealer.creditLimit)} limit` },
